			...
			int Ones, int iter, int X, int Y, int Z, int n)
{
	...
	#pragma omp parallel for schedule(static, 512) num_threads(64) firstprivate(arrayX, arrayY, seed, n)
   	for(i = 0; i < n; i++){
   		arrayX[i] += 1 + 5*rand2(seed, i);
   		arrayY[i] += -2 + 2*rand2(seed, i);
   	}

   	#pragma omp parallel for   schedule(static, 512) num_threads(64) firstprivate(n, iter, Y, Z) private(index_X, index_Y, j)
   	for(i = 0; i<n; i++){
   		for(j = 0; j < Ones; j++){
   			int k = j*2;
  	...
  	#pragma omp parallel for   schedule(static, 2048) num_threads(16) firstprivate(probability)
   	for(i = 0; i<n; i++){
   		probability[i] = 0;

   		for(j = 0; j < Ones; j++) {
   			probability[i] += (pow((array[index[i*Ones + j]] - 100),2) -
   							  pow((array[index[i*Ones + j]]-228),2));
   		}
   		probability[i] = probability[i]/((double) 50*Ones);
   	}
}
...
void func2(double *weights, double *probability, int n)
{
	...
	double sumWeights=0;
  	...
  	#pragma omp parallel for   schedule(static, 512) num_threads(64)
	for(i = 0; i < n; i++)
   		weights[i] = weights[i] * exp(probability[i]);

 	#pragma omp parallel for   reduction(+:sumWeights) schedule(static, 512) num_threads(64)
   	for(i = 0; i < n; i++)
...
void func5(double *x_j, double *y_j, double *arrayX, double *arrayY, double *weights, double *cfd, double *u, int n)
{
	...
	#pragma omp parallel for schedule(static, 512) num_threads(64) private(i)
	for(j = 0; j < n; j++){

...
#pragma omp parallel for num_threads(NUM_THREADS) firstprivate (n, weights, weight, arrayX, arrayY, dx, dy) private(i)
    for(i = 0; i < n; i++)
    {
        weights[i] = weight;
        arrayX[i] = dx;
...
#pragma omp parallel for num_threads(NUM_THREADS) firstprivate(n, seed, arrayX, arrayY) private(i)
    for(i = 0; i < n; i++)
    {
...
#pragma omp parallel for num_threads(NUM_THREADS) firstprivate(Ones, n, iter, Y, Z, max_size, objxy, index, array) private(i, j, k, index_X, index_Y, arrayIndex, arrayValue, indexValue)
    for(i = 0; i < n; i++)
    {
        arrayIndex = i * Ones;
    #pragma omp simd
...
#pragma omp parallel for num_threads(NUM_THREADS) firstprivate(n, weights, probability) private(i, weight) reduction(+:sumWeights)
    for(i = 0; i < n; i++)
    {
...
#pragma omp parallel for num_threads(NUM_THREADS) firstprivate(n, arrayX, arrayY, weights) private(i, weight) reduction(+:estimate_x, estimate_y)
...
#pragma omp parallel for num_threads(NUM_THREADS) firstprivate(n, m, cfd, u, arrayX, arrayY, x_j, y_j) private(i)
    for(j = 0; j < n; j++)
    {
...
#pragma omp parallel for num_threads(NUM_THREADS) firstprivate(n, weight, arrayX, arrayY, weights, x_j, y_j) 
    for(i = 0; i < n; i++)
    {
        arrayX[i] = x_j[i];
        arrayY[i] = y_j[i];
